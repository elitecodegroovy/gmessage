...
func ProcessHelperCommands(cmd *flag.FlagSet) (showVersion bool, showHelp bool, err error) {
	if len(cmd.Args()) > 0 {
		arg := cmd.Args()[0]
...
func (s *Server) Start() {
	...
	if opts.PidFile != _EMPTY_ {
		if err := s.logPid(); err != nil {
			PrintNExit(fmt.Sprintf("Could not write pidfile: %v\n", err))
		}
	}
